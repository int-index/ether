...
ethered _proxy = coerce

instance MB.MonadBase b m => MB.MonadBase b (WrappedEther tag m) where
  liftBase a = WrapEther (MB.liftBase a)

instance MC.MonadBaseControl b m => MC.MonadBaseControl b (WrappedEther tag m) where
  type StM (WrappedEther tag m) a = MC.StM m a
  liftBaseWith a = WrapEther (MC.liftBaseWith (a . coerce))
  restoreM a = WrapEther (MC.restoreM a)

instance MonadReader tag r m => Class.MonadReader r (WrappedEther tag m) where
    ask = WrapEther $ ask (Proxy :: Proxy tag)
    local f = WrapEther . local (Proxy :: Proxy tag) f . coerce

instance MonadReader tag r m => MonadReader tag r (WrappedEther tag' m) where
    ask t = WrapEther $ ask t
    local t f = WrapEther . local t f . coerce
